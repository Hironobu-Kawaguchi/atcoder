...
void Yamanobori() {
	...
	while (clock() - ti < TIMELIMIT) {
		...
		int old_x = X[t], new_x = X[t] + RandInt(-1, 1);
		// int old_y = Y[t], new_y = Y[t] + RandInt(-9, 9);
		int old_y = Y[t], new_y = Y[t] + RandInt(-1, 1);
		// int old_h = H[t], new_h = H[t] + RandInt(-19, 19);
        int h_limit = 14;
        if (CurrentScore >= 199900000) h_limit = 1;
        else if (CurrentScore >= 199500000) h_limit = 7;
		int old_h = H[t], new_h = H[t] + RandInt(-h_limit, h_limit);
