...
class Solver:
    ...
    def __init__(self, L: int, N: int, S: int, landing_pos: List[Pos], landing: List[List[bool]]):
        ...
        self.interval_num = 2 if self.interval > 32 else 32 // self.interval + 1   # 10箇所取れば，2回の計測で10^2==100通りを判別可能，３回で10^3==1000通りを判別可能
        ...
        self.max_temperture = min(1000, int(self.S * 2.0) + 40)
        ...
        print(f"pass_flg={self.pass_flg} interval={self.interval} interval_num={self.interval_num}", file=sys.stderr)
