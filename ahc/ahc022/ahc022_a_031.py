...
class Solver:
    ...
    def __init__(self, L: int, N: int, S: int, landing_pos: List[Pos], landing: List[List[bool]]):
        ...
        print(f"max_temperture={self.max_temperture}", file=sys.stderr)
    ...
    def generate_noise_grid(self, max_value=1000):
        ...
        scale_factor = int(self.L * 0.55)  # この値を変更すると、ノイズの特性が変わります
        small_grid = np.random.randint(0, max_value + 1, size=(scale_factor, scale_factor))
        ...
        upscaled_grid = np.clip(upscaled_grid, 0, max_value)
