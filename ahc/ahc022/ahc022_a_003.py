...
class Solver:
    ...
    def __init__(self, L: int, N: int, S: int, landing_pos: List[Pos], landing: List[List[bool]]):
        ...
        self.interval = min(self.S + 2, 1000 // self.N)        # 配置間隔: Sが小さい時は小さい間隔でコストを抑える
        self.measure_count = min(10000 // self.N, math.ceil((2 * self.z * self.S / self.interval) ** 2) + 2)        # 計測回数
    ...
    def _create_temperature(self) -> List[List[int]]:
        ...
        for i, pos in enumerate(self.landing_pos):
            ...
            temperature[pos.y][pos.x] = (i+1) * self.interval
