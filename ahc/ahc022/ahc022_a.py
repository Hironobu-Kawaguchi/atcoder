...
class Solver:
    ...
    def __init__(self, L: int, N: int, S: int, landing_pos: List[Pos], interval: int = None, interval_num: int = None, z: float = None):
        ...
        if z is not None:
            ...
            # self.z = 2.58 / (self.L / 50)   # 95% confidence interval
        ...
        if interval is not None:
            ...
            self.interval = min(math.ceil(self.S * self.z), 1000)
            # self.interval = min(math.ceil(self.S * self.z), 500)
            # self.interval = min(math.ceil(self.S * self.z), 1000) if self.S < 100 else self.S
            # self.interval = min(math.ceil(self.S * self.z), 1000) if self.S <= threshold else min(math.ceil(self.S * 1.3), 1000)
        if interval_num is not None:
            ...
            self.interval_num = 2 if self.interval > threshold else threshold // self.interval + 1   # 10箇所取れば，2回の計測で10^2==100通りを判別可能，３回で10^3==1000通りを判別可能
        ...
        # self.max_temperature = min(500, self.interval * (self.interval_num - 1))
        ...
        self.squared_error = [[[] for _ in range(self.N)] for _ in range(self.N)]

        # self.n_observations = len(self.move_list)
        # self.n_observations = min(len(self.move_list), math.ceil(math.log(2**(len(self.move_list))+1, self.interval_num)))
        self.n_observations = max(min(len(self.move_list), math.ceil(math.log(2**(len(self.move_list))+1, self.interval_num))), 5)
    ...
    def solve(self) -> None:
        ...
        print(f"# Placement cost = {self.placement_cost}")
        print(f"# Placement cost = {self.placement_cost}", file=sys.stderr)
        self.squared_error = self._calc_squared_error(temperature)
        measurement_count, measurement_cost = self._predict_measurement_count()
        print(f"# predict Measurement count = {measurement_count} Measurement cost = {measurement_cost}")
        print(f"# predict Measurement count = {measurement_count} Measurement cost = {measurement_cost}", file=sys.stderr)
        self.judge.set_temperature(temperature)
        estimate = self._predict(temperature)
        self.judge.answer(estimate)

    def _predict_measurement_count(self) -> int:
        # print(f"squared_error {self.squared_error}", file=sys.stderr)
        measurement_cost = 0
        squared_error_sum = [[0]*self.N for _ in range(self.N)]
        now = [[-1]*self.N for _ in range(self.N)]
        for measurement_count in range(self.N, 10000, self.N):
            move_idx = (measurement_count//self.N - 1) % self.n_observations
            measurement_cost += 100 * (10 + abs(self.move_list[move_idx][0]) + abs(self.move_list[move_idx][1])) * self.N
            for i in range(self.N):
                for j in range(i+1, self.N):
                    squared_error_sum[i][j] += self.squared_error[i][j][move_idx]
                    now[i][j] = 1 - squared_error_sum[i][j] / (self.S * self.S * measurement_count / self.N)
            r2_max = 0
            for i in range(self.N):
                for j in range(i+1, self.N):
                    if now[i][j] > r2_max:
                        ...
                        i_max, j_max = i, j
            if r2_max < 0.5:
                # print(f"r2 = {r2_max}", file=sys.stderr)
                return measurement_count, measurement_cost
        # print(f"r2 = {r2_max}", file=sys.stderr)
        # print(f"squared_error {now[i_max][j_max]} i:{i_max} j:{j_max}", file=sys.stderr)
        # print(f"squared_error[i_max][j_max] {self.squared_error[i_max][j_max]}", file=sys.stderr)
        # print(f"squared_error_sum[i_max][j_max] {squared_error_sum[i_max][j_max]}", file=sys.stderr)
        measurement_cost = max(10000, measurement_cost)
        return measurement_count, measurement_cost
    ...
    def _calc_placement_cost(self, temperature: List[List[int]]) -> int:
        ...
        for i in range(self.L):
            for j in range(self.L):
                cost += (temperature[i][j] - temperature[(i+1)%self.L][j])**2 + (temperature[i][j] - temperature[i][(j+1)%self.L])**2
        return cost
    
    def _squared_error(self, i, j, y, x, temperature: List[List[int]]) -> float:
        niy, nix = (self.landing_pos[i].y + y + self.L)%self.L, (self.landing_pos[i].x + x + self.L)%self.L
        njy, njx = (self.landing_pos[j].y + y + self.L)%self.L, (self.landing_pos[j].x + x + self.L)%self.L
        return (temperature[niy][nix] - temperature[njy][njx])**2

    def _calc_squared_error(self, temperature: List[List[int]]):
        squared_error = [[[] for _ in range(self.N)] for _ in range(self.N)]
        for i in range(self.N):
            for j in range(i+1, self.N):
                for y, x in self.move_list:
                    squared_error[i][j].append(self._squared_error(i, j, y, x, temperature))
    ...
    def _create_temperature(self) -> List[List[int]]:
        ...
        iter = 0
        # while time.time() - time_start < 3.0:   # TLE
        # while time.time() - time_start < 2.0:   # TLE
        while time.time() - time_start < 1.5:
            iter += 1
            temperature = [[random.choice(temp_list) for _ in range(self.L)] for _ in range(self.L)]
            placement_cost = self._calc_placement_cost(temperature)
            self.squared_error = self._calc_squared_error(temperature)
            measurement_count, measurement_cost = self._predict_measurement_count()
            if placement_cost + measurement_cost < best_cost:
                best_cost = placement_cost + measurement_cost
                best_temperature = temperature
        temperature = best_temperature
        print(f"# iter={iter} best_cost = {best_cost}")
        print(f"# iter={iter} best_cost = {best_cost}", file=sys.stderr)
        ...
        for i in range(self.N):
            for j in range(self.n_observations):
                ...
                self.fixed_temps[ny][nx] = True   # 温度を固定
        ...
        # temperature = [[self.max_temperature//2 for _ in range(self.L)] for _ in range(self.L)]
        ...
        # # 2値の場合，差を小さくする
        # if self.interval_num==2 and self.S < 600:
        #     for y in range(self.L):
        #         for x in range(self.L):
        #             flg = False
        #             flgs = [False] * 2
        #             for vy, vx in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
        #                 ny, nx = (y + vy + self.L)%self.L, (x + vx + self.L)%self.L
        #                 if not self.fixed_temps[ny][nx]: continue
        #                 if temperature[y][x] != temperature[ny][nx]:
        #                     flg = True
        #                 for i in range(2):
        #                     if temperature[ny][nx] == temp_list[i]:
        #                         flgs[i] = True
        #             if flgs[0] and flgs[1]:
        #                 temperature[y][x] = int(np.mean(temp_list))
        #             elif flg and flgs[0]:
        #                 temperature[y][x] = temp_list[0]
        #             elif flg and flgs[1]:
        #                 temperature[y][x] = temp_list[1]
        ...
        self.placement_cost = self._calc_placement_cost(temperature)
        return temperature
        # # if self.pass_flg:
        # if self.S > 500:
