...
def beam_search():
    ...
    for bi in range(MAX_MOVE):
        ...
        #     dbeam_strset[last5str] = beam[bi][bj_list[0]].get_str
        # for bj in range(len(beam[bi])):
        for last5str, bj_list in dbeam.items():
            for c, pos_list in dA.items():
                new5str = last5str[-4:] + c
                if len(new5str) == 5 and new5str in t:
                    new_get_str = beam[bi][bj_list[0]].get_str | set([new5str])
                else:
                    new_get_str = beam[bi][bj_list[0]].get_str
        ...
        if bi <= 10:    # 10手までの確認
            for j in range(10):
                print(f"bi-j:{bi}-{j}, score:{beam[-1][j].score}, cost:{beam[-1][j].cost}, get_str:{len(beam[-1][j].get_str)}, last5str:{beam[-1][0].last5str}", file=sys.stderr)
