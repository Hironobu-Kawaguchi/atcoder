...
while ci.cnt_queries < Q and time.time() - time_start < 1.7:
    ...
    # move_set_small = set([random.choice(list(HappyBags[idx_small]))])
    move_set_small = set()
    print("# move set small", move_set_small, flush=True)
    r_sorted_indices = merge_sort(list(HappyBags[idx_big]), ci)
    l, r = 0, len(r_sorted_indices)
    while l+1 < r:
        m = (l+r)//2
        move_set = set([r_sorted_indices[m]])
        ...
        if ci.compare(new_small_set, new_big_set) >= 0:
            ...
            r = m
    if l+1!=r or r==len(r_sorted_indices):
        break
    else:
        move_set = set([r_sorted_indices[l]])
        print("# move", r_sorted_indices[l], flush=True)
