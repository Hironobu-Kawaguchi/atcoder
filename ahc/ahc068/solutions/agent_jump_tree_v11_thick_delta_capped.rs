        ...
        while remaining > 1 {
            ...
            while at + 1 < path.len() {
                ...
                for jump in (1..=straight).rev() {
                    ...
                    // First pass: evaluate only thin (thickness=1) windows across offsets.
                    ...
                    for offset in 0..=max_offset {
                        ...
                        if swapped_cell(line_op, path[at], n) != Some(path[at + jump])
                            || !jump_window_active(line_op, &active, n)
                            || !rectangle_open(
                                line_op.r, line_op.c, line_op.h, line_op.w, vertical, horizontal,
                            )
                        {
                            continue;
                        }
                        ...
                        {
                            best_thin = Some((key, line_op, offset));
                        }
                    }
                    // Second pass: thicken only the winning thin window.
                    let mut best_window = best_thin.map(|(key, op, _)| (key, op));
                    if matches!(policy, JumpPolicy::ThickDelta) {
                        if let Some((_, line_op, offset)) = best_thin {
                            for thickness in 2..=4usize {
