...
/// Builds the trace-monoid DAG of adjacent swaps. Only consecutive events on
/// the same cell are ordered; disjoint events commute. A ready layer is a
/// matching, whose parallel swaps can be fused into wall-free rectangles.
fn compress_fallback_dag(
    raw: &[Operation],
    n: usize,
    vertical: &[Vec<u8>],
    horizontal: &[Vec<u8>],
    ...
    fn pair(op: Operation, n: usize) -> (usize, usize) {
        let a = op.r * n + op.c;
        (a, if op.direction == b'H' { a + 1 } else { a + n })
    }
    fn emit_layer(
        layer: &[usize],
        raw: &[Operation],
...
fn choose_compressed_tail(
    ...
    let original = compress_fallback(raw, n, vertical, horizontal);
    let dag = compress_fallback_dag(raw, n, vertical, horizontal);
    let original_valid = replay_sorted(state, &original, n, vertical, horizontal);
    let dag_valid = replay_sorted(state, &dag, n, vertical, horizontal);
    match (original_valid, dag_valid) {
        (true, true) if dag.len() < original.len() => dag,
        (true, _) => original,
        (false, true) => dag,
        (false, false) => {
    ...
    for (root_index, &root) in roots.iter().enumerate() {
        for (order_index, order) in ORDERS.iter().enumerate() {
            ...
            for checkpoint in &checkpoints {
                ...
                let fallback =
                    choose_compressed_tail(&checkpoint.board, &raw, n, &vertical, &horizontal);
