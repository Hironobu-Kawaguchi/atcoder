...
mod tree_v4 {
    ...
    const SEARCH_SECONDS: f64 = 0.173_333;
...
mod region_v5 {
    ...
    const ROOM_SECONDS: f64 = 0.050_000;
    const LINE_SECONDS: f64 = 0.000_833;
    const DOOR_SECONDS: f64 = 0.018_333;
    const MACRO_SECONDS: f64 = 0.093_333;
...
mod zero_v6 {
    ...
    const SEARCH_SECONDS: f64 = 0.113_333;
    const SEARCH_MOVE_LIMIT: usize = 20_000;
    const CHECK_INTERVAL: usize = 20;
    const CHECKPOINT_LIMIT: usize = 5;
    const LINE_BEAM_WIDTH: usize = 200;
    const LINE_BEAM_DEPTH: usize = 14;
    const LINE_ROUTE_SECONDS: f64 = 0.226_667;
...
fn diagnostic(label: &str, candidate: &str, started: Instant) {
    if DIAGNOSTICS {
        eprintln!(
            "calibration branch={label} output_operations={} elapsed_ms={:.3}",
...
fn solve(input: &str) -> String {
    ...
    if problem.wall_count == 0 {
        ...
        let candidate = zero_v6::solve(input);
        diagnostic("zero_v6", &candidate, branch_started);
        consider(&mut best, candidate, &problem);
        if best.is_none() {
            let branch_started = Instant::now();
            let candidate = tree_v4::solve(input);
            diagnostic("tree_v4_fallback", &candidate, branch_started);
            consider(&mut best, candidate, &problem);
        }
    } else {
        let branch_started = Instant::now();
        let candidate = tree_v4::solve(input);
        diagnostic("tree_v4", &candidate, branch_started);
        consider(&mut best, candidate, &problem);
        ...
        if started.elapsed().as_secs_f64() < 0.273_333 || problem.wall_count >= 80 {
            let branch_started = Instant::now();
            let candidate = region_v5::solve(input);
            diagnostic("region_v5", &candidate, branch_started);
            consider(&mut best, candidate, &problem);
        }
    }
