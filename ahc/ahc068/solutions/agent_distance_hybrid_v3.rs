...
const FAST_PHASE_SEC: f64 = 0.62;
const SEARCH_END_SEC: f64 = 1.52;
const PREFIX_LIMIT: usize = 12_000;
const CHECK_INTERVAL: usize = 24;
const ROLLOUT_WIDTH: usize = 10;
const MAX_RECT_SIDE: usize = 6;
...
fn solve(input: &str) -> String {
    ...
    // Phase 1: cheaply descend the distance energy, but retain only checkpoints
    // whose exact best completion improves the emitted operation count.
    ...
    while prefix.len() < PREFIX_LIMIT && start.elapsed().as_secs_f64() < FAST_PHASE_SEC {
        let Some((value, op)) = candidates
            .iter()
            .copied()
            .map(|op| (energy_delta(op, &board, &distance, n), op))
        ...
        prefix.push(op);
        if prefix.len() % CHECK_INTERVAL == 0 || value <= -12 {
            let (remaining, _) = best_fallback(&plans, &board, &position, n);
            let total = prefix.len() + remaining;
            if total < best_total {
                best_total = total;
    ...
    let (remaining, _) = best_fallback(&plans, &board, &position, n);
    if prefix.len() + remaining < best_total {
        best_total = prefix.len() + remaining;
        best_board = board;
        best_position = position;
        best_prefix = prefix;
    }

    // Phase 2: start at the best checkpoint and directly minimize
    // one emitted move plus the exact residual fallback.  Energy is only a
    // shortlist filter; the accept/reject decision uses actual operation count.
    board = best_board;
    ...
    while prefix.len() < PREFIX_LIMIT && start.elapsed().as_secs_f64() < SEARCH_END_SEC {
        let mut shortlist = candidates
            ...
            .map(|op| (energy_delta(op, &board, &distance, n), op))
            .collect::<Vec<_>>();
        shortlist.sort_unstable_by_key(|&(value, _)| value);
        shortlist.truncate(ROLLOUT_WIDTH.min(shortlist.len()));
