...
struct TreePlan {
    ...
    depth: Vec<usize>,
    distance: Vec<u16>,
    jump_distance: Vec<u16>,
    jump_aware: bool,
    straight_policy: Option<JumpPolicy>,
}
...
impl TreePlan {
    fn new(
        ...
        Self::with_style(n, root, order, vertical, horizontal, TreeStyle::Bfs)
    }
    ...
    fn new_straight(
        ...
        let mut result =
            Self::with_style(n, root, order, vertical, horizontal, TreeStyle::StraightBfs);
        result.straight_policy = Some(policy);
        result
    }
    ...
    fn with_style(
        ...
        let size = n * n;
        let mut mesh = vec![Vec::new(); size];
        for cell in 0..size {
            for direction in 0..4 {
                if let Some(to) = neighbor(cell, direction, n, vertical, horizontal) {
                    mesh[cell].push(to);
                }
            }
        }
        ...
        match style {
            ...
            TreeStyle::StraightBfs => {
                let mut grid_distance = vec![usize::MAX; size];
                grid_distance[root] = 0;
                let mut queue = VecDeque::from([root]);
                while let Some(v) = queue.pop_front() {
                    for &direction in order {
                        let Some(to) = neighbor(v, direction, n, vertical, horizontal) else {
                        ...
                        if grid_distance[to] == usize::MAX {
                            grid_distance[to] = grid_distance[v] + 1;
                            queue.push_back(to);
                        }
                    }
                }
                assert!(grid_distance.iter().all(|&d| d != usize::MAX));

                // Dynamic programming on shortest-path layers chooses the
                // fewest-turn root path. Equal-turn alternatives continue the
                // current direction and enter the longest open corridor.
                let mut turns = vec![usize::MAX; size];
                let mut incoming = vec![usize::MAX; size];
                let mut straight_run = vec![0usize; size];
                parent[root] = root;
                turns[root] = 0;
                let max_depth = *grid_distance.iter().max().unwrap();
                for layer in 1..=max_depth {
                    for cell in 0..size {
                        if grid_distance[cell] != layer {
                            continue;
                        }
                        let mut best: Option<(
                        ...
                        for (rank, &direction) in order.iter().enumerate() {
                            let Some(from) =
                                neighbor(cell, (direction + 2) % 4, n, vertical, horizontal)
                            else {
                                continue;
                            };
                            if grid_distance[from] + 1 != layer {
                                continue;
                            }
                            let changes_direction = from != root && incoming[from] != direction;
                            let candidate_turns = turns[from] + usize::from(changes_direction);
                            let candidate_run = if incoming[from] == direction {
                                straight_run[from] + 1
                            } else {
                                1
                            };
                            let corridor =
                                forward_corridor(cell, direction, n, vertical, horizontal);
                            let side_width =
                                perpendicular_clearance(cell, direction, n, vertical, horizontal);
                            let key = (
                                candidate_turns,
                                n.saturating_sub(candidate_run),
                                n.saturating_sub(corridor),
                                (2 * n).saturating_sub(side_width),
                                rank,
                            );
                            if best
                                .as_ref()
                                .is_none_or(|(best_key, _, _, _)| key < *best_key)
                            {
                                best = Some((key, from, direction, candidate_run));
                            }
                        }
                        let (_, from, direction, run) =
                            best.expect("every non-root shortest-path layer has a predecessor");
                        parent[cell] = from;
                        depth[cell] = depth[from] + 1;
                        turns[cell] =
                            turns[from] + usize::from(from != root && incoming[from] != direction);
                        incoming[cell] = direction;
                        straight_run[cell] = run;
                        add_edge(&mut tree, from, cell);
                    }
                }
            }
        }
        ...
        let jump_aware = matches!(style, TreeStyle::StraightBfs);
        let jump_distance = if jump_aware {
            ideal_jump_distances(&tree, n, 10)
        } else {
            Vec::new()
        };
        Self {
    ...
    fn complete_jump(
        ...
        let mut active = vec![true; size];
        let mut degree: Vec<usize> = self.tree.iter().map(Vec::len).collect();
        let mut active_mesh_degree: Vec<u8> = self
            .mesh
            .iter()
            .map(|neighbors| neighbors.len() as u8)
            .collect();
        let route_distance = if self.jump_aware {
            &self.jump_distance
        } else {
            &self.distance
        };
        ...
        while remaining > 1 {
            ...
            for candidate in 0..size {
                ...
                let d = route_distance[position[candidate] * size + candidate];
                let width = if self.jump_aware {
                    active_mesh_degree[candidate]
                } else {
                    0
                };
                let index_tie = leaf == usize::MAX
                    || (prefer_larger && candidate > leaf)
                    || (!prefer_larger && candidate < leaf);
                let improves = d < best_distance
                    || (d == best_distance
                        && (width < best_width || (width == best_width && index_tie)));
                if improves {
                    best_distance = d;
                    best_width = width;
                    leaf = candidate;
            ...
            active[leaf] = false;
            if self.jump_aware {
                for &to in &self.mesh[leaf] {
                    if active[to] {
                        active_mesh_degree[to] -= 1;
                    }
                }
                active_mesh_degree[leaf] = 0;
            }
            remaining -= 1;
            for &to in &self.tree[leaf] {
                if active[to] {
...
fn direction_index_between(a: usize, b: usize, n: usize) -> usize {
    match direction_between(a, b, n) {
        (-1, 0) => 0,
        (0, 1) => 1,
        (1, 0) => 2,
        (0, -1) => 3,
        _ => panic!("tree edge must join orthogonally adjacent cells"),
    }
}
...
fn forward_corridor(
    ...
    let mut result = 0;
    let mut cell = start;
    while let Some(to) = neighbor(cell, direction, n, vertical, horizontal) {
        result += 1;
        cell = to;
    }
    result
...
fn perpendicular_clearance(
    ...
    1 + forward_corridor(start, (direction + 1) % 4, n, vertical, horizontal)
        + forward_corridor(start, (direction + 3) % 4, n, vertical, horizontal)
}

/// Ideal number of `k<=max_jump` straight jumps along each unique tree path.
/// It deliberately ignores active-window and boundary constraints: as a leaf
/// ordering potential it is a closer O(1) proxy than raw tree-edge distance.
fn ideal_jump_distances(tree: &[Vec<usize>], n: usize, max_jump: usize) -> Vec<u16> {
    assert!(max_jump > 0);
    let size = tree.len();
    let mut result = vec![u16::MAX; size * size];
    for source in 0..size {
        result[source * size + source] = 0;
        let mut stack = vec![(source, usize::MAX, usize::MAX, 0usize, 0u16)];
        while let Some((cell, from, last_direction, run, jumps)) = stack.pop() {
            for &to in &tree[cell] {
                if to == from {
                    continue;
                }
                let direction = direction_index_between(cell, to, n);
                let (next_run, next_jumps) = if direction == last_direction && run < max_jump {
                    (run + 1, jumps)
                } else {
                    (1, jumps + 1)
                };
                result[source * size + to] = next_jumps;
                stack.push((to, cell, direction, next_run, next_jumps));
            }
        }
    }
    debug_assert!(result.iter().all(|&d| d != u16::MAX));
    result
}
...
fn choose_completion(
    ...
    let policies = match plan.straight_policy {
        Some(policy) => [Some(policy), None],
        None => [Some(JumpPolicy::Distance), Some(JumpPolicy::Clearance)],
    };
...
fn solve(input: &str) -> String {
    ...
    for (root_index, &root) in roots.iter().enumerate() {
        for (order_index, order) in ORDERS.iter().enumerate() {
            // Keep four exact V9 BFS families and replace the four mirrored
            // families with shortest-edge, straight-priority trees. The plan
            // count remains 9 * 8; each straight tree evaluates one of the
            // two long-window policies so total tail work is lower than V9.
            let plan = if order_index < 4 {
                TreePlan::new(n, root, order, &vertical, &horizontal)
            } else {
                let policy = if order_index % 2 == 0 {
                    JumpPolicy::Distance
                } else {
                    JumpPolicy::Clearance
                };
                TreePlan::new_straight(n, root, order, &vertical, &horizontal, policy)
            };
