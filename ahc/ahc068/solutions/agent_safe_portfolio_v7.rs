...
fn region_decompose(
    ...
    for &(_, r, c, h, w) in &rectangles {
        if h * w < 12 || !(r..r + h).all(|rr| (c..c + w).all(|cc| owner[rr * n + cc] == usize::MAX))
        {
            continue;
        }
        let id = rooms.len();
    ...
    for stage in 0..3 {
        for &room in &rooms {
            ...
            for line in 0..line_count {
                ...
                for (start, half) in
                    region_line_moves(&initial_line, &cells, vertical, room.id, &owner, n, global)
                {
...
fn validates(
    ...
    for &op in operations {
        ...
        if !dimensions_are_valid || !rectangle_open(op.r, op.c, op.h, op.w, vertical, horizontal) {
            return false;
        }
        apply(op, &mut board, &mut position, n);
    }
