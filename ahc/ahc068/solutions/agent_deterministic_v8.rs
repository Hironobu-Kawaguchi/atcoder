...
fn solve_core(
    ...
    while prefix.len() < search_move_limit
        && (!use_time_limit || start.elapsed().as_secs_f64() < SEARCH_SECONDS)
    {
    ...
    for (root_index, &root) in roots.iter().take(root_limit).enumerate() {
        for (order_index, order) in ORDERS.iter().take(order_limit).enumerate() {
            let plan = TreePlan::new(n, root, order, &vertical, &horizontal);
            for checkpoint in &checkpoints {
                let raw = plan.complete(&checkpoint.board, n, (root_index + order_index) % 2 == 1);
...
fn parse_operations(output: &str) -> Vec<Operation> {
    let mut scanner = Scanner(output.split_whitespace());
    let tokens = output.split_whitespace().count();
    assert_eq!(tokens % 5, 0);
    (0..tokens / 5)
        .map(|_| Operation {
            direction: scanner.next::<String>().as_bytes()[0],
            r: scanner.next(),
            c: scanner.next(),
            h: scanner.next(),
            w: scanner.next(),
...
fn cancel_identical_pairs(operations: &[Operation]) -> Vec<Operation> {
    let mut stack = Vec::with_capacity(operations.len());
    for &op in operations {
        if stack.last() == Some(&op) {
            stack.pop();
        } else {
            stack.push(op);
        }
...
fn parse_instance(input: &str) -> (usize, Vec<usize>, Vec<Vec<u8>>, Vec<Vec<u8>>) {
    let mut scanner = Scanner(input.split_whitespace());
    let n: usize = scanner.next();
    let board = (0..n * n).map(|_| scanner.next()).collect();
    let vertical = (0..n)
        .map(|_| scanner.next::<String>().into_bytes())
        .collect();
    let horizontal = (0..n - 1)
        .map(|_| scanner.next::<String>().into_bytes())
        .collect();
    (n, board, vertical, horizontal)
}

fn transpose_instance(input: &str) -> String {
    let (n, board, vertical, horizontal) = parse_instance(input);
    let mut transposed = vec![0; n * n];
    for r in 0..n {
        for c in 0..n {
            let card = board[r * n + c];
            let (target_r, target_c) = (card / n, card % n);
            transposed[c * n + r] = target_c * n + target_r;
        }
    }
    let mut output = String::new();
    ...
    for r in 0..n {
        for c in 0..n {
            write!(output, "{} ", transposed[r * n + c]).unwrap();
        }
        writeln!(output).unwrap();
    }
    // A horizontal adjacency after transposition was vertical before it.
    for r in 0..n {
        for c in 0..n - 1 {
            output.push(horizontal[c][r] as char);
        }
        output.push('\n');
    }
    // A vertical adjacency after transposition was horizontal before it.
    for r in 0..n - 1 {
        for c in 0..n {
            output.push(vertical[c][r] as char);
        }
        output.push('\n');
    }
    output
...
fn inverse_transpose(op: Operation) -> Operation {
    Operation {
        direction: if op.direction == b'H' { b'V' } else { b'H' },
        r: op.c,
        c: op.r,
        h: op.w,
        w: op.h,
...
fn replay(input: &str, operations: &[Operation]) -> bool {
    let (n, mut board, vertical, horizontal) = parse_instance(input);
    ...
    for &op in operations {
        let legal_dimensions = op.h > 0
            && op.w > 0
            && op.r + op.h <= n
            && op.c + op.w <= n
            && ((op.direction == b'H' && op.w % 2 == 0) || (op.direction == b'V' && op.h % 2 == 0));
        if !legal_dimensions || !rectangle_open(op.r, op.c, op.h, op.w, &vertical, &horizontal) {
            return false;
        }
        apply(op, &mut board, &mut position, n);
    }
...
fn operations_to_string(operations: &[Operation]) -> String {
    let mut output = String::new();
    for op in operations {
        writeln!(
            output,
...
fn solve(input: &str) -> String {
    // Mandatory, unchanged V4 candidate.
    let baseline_text = solve_core(input, SEARCH_MOVE_LIMIT, 9, 8, true);
    let baseline_raw = parse_operations(&baseline_text);
    let mut best = cancel_identical_pairs(&baseline_raw);
    assert!(replay(input, &best));

    // Optional deterministic transpose portfolio. Its work is fixed at 96
    // heuristic iterations and 3 roots x 2 direction orders.
    let transposed_input = transpose_instance(input);
    let transposed_text = solve_core(&transposed_input, 96, 3, 2, false);
    let transposed_raw: Vec<Operation> = parse_operations(&transposed_text)
        .into_iter()
        .map(inverse_transpose)
        .collect();
    let transposed = cancel_identical_pairs(&transposed_raw);
    if transposed.len() < best.len() && replay(input, &transposed) {
        best = transposed;
    }
    assert!(replay(input, &best));
    operations_to_string(&best)
}
