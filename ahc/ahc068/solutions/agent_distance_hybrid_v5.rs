...
const ROUTING_MOVE_LIMIT: usize = 16_000;
const ROUTING_ROUNDS: usize = 6;
...
fn route_segment(
    ...
    let mut added = 0;
    while output.len() < ROUTING_MOVE_LIMIT {
        let mut best: Option<((isize, isize, isize), usize, usize)> = None;
        for start in 0..cells.len() {
            for half in 1..=(cells.len() - start) / 2 {
                let mut error_delta = 0isize;
                let mut exact_delta = 0isize;
                for offset in 0..half {
                    let a = cells[start + offset];
                    let b = cells[start + half + offset];
                    let x = board[a];
                    let y = board[b];
                    let coordinate = |cell: usize| {
                        if direction == b'H' {
                            cell % n
                        } else {
                            cell / n
                    ...
                    error_delta += coordinate(b).abs_diff(target(x)) as isize
                        + coordinate(a).abs_diff(target(y)) as isize
                        - coordinate(a).abs_diff(target(x)) as isize
                        - coordinate(b).abs_diff(target(y)) as isize;
                    exact_delta += (y == a) as isize + (x == b) as isize
                        - (x == a) as isize
                        - (y == b) as isize;
                }
                if error_delta < 0 || (error_delta == 0 && exact_delta > 0) {
                    let key = (error_delta, -exact_delta, -(half as isize));
                    let improves = match best.as_ref() {
                        Some(&(old, _, _)) => key < old,
                        None => true,
                    };
                    if improves {
                        best = Some((key, start, half));
                    }
                }
            }
        }
        let Some((_, start, half)) = best else {
            break;
        };
        let first = cells[start];
        let op = if direction == b'H' {
            Operation {
                direction,
                r: first / n,
                c: first % n,
    ...
    if direction == b'H' {
        for r in 0..n {
            ...
            while left < n {
                ...
                route_segment(&cells, direction, board, position, n, output);
                left = right;
            }
        }
    } else {
...
fn structured_checkpoints(
    initial: &[usize],
    first_direction: u8,
    n: usize,
    vertical: &[Vec<u8>],
    horizontal: &[Vec<u8>],
) -> Vec<(Vec<usize>, Vec<Operation>)> {
    let mut board = initial.to_vec();
    ...
    for phase in 0..2 * ROUTING_ROUNDS {
        let direction = if phase % 2 == 0 {
            first_direction
        } else if first_direction == b'H' {
            b'V'
        } else {
            b'H'
        };
        let added = route_dimension(
            direction,
            &mut board,
            &mut position,
            n,
        ...
        result.push((board.clone(), output.clone()));
        if added == 0 && phase > 0 {
            break;
        }
        if output.len() >= ROUTING_MOVE_LIMIT {
            break;
        }
    }
    result
...
fn solve(input: &str) -> String {
    ...
    let size = n * n;
    let board: Vec<usize> = (0..size).map(|_| scanner.next()).collect();
    ...
    let connectivity = all_distances(&graph);
    assert!(connectivity.iter().all(|&d| d != u16::MAX));
    ...
    save_checkpoint(&mut checkpoints, &board, &[], &quick_plan, n);

    // Two deterministic row/column routing schedules.  A phase consists only
    // of equal-block swaps inside maximal passable 1-D segments; no global
    // rectangle scan or stochastic search is used.
    for first_direction in [b'H', b'V'] {
        for (routed_board, routed_prefix) in
            structured_checkpoints(&board, first_direction, n, &vertical, &horizontal)
        {
            save_checkpoint(
                &mut checkpoints,
                &routed_board,
